...
namespace bank_app.UI.Pages
{
    public class ClientDashboard : Page
    {
        internal override UIComponent LoadPage()
        {
            ...
            // fetches the current users all accounts and stores them in a list
            var currentUsersAccounts = AccountManager.GetAllAccounts(CurrentUser.UserId);
            ...
            for (int i = 0; i < AccountManager.GetAllAccounts(CurrentUser.UserId).Count; i++)
            {
                ...
                {
                    column2[i] = $"{currentUsersAccounts[i].Balance.ToString("F2")} {currentUsersAccounts[i].AccountCurrency}";
                }
                else
                {
